...
namespace Webtools {
    ...
    public class APIBrowser : IHttpHandler {
        ...
        /// <returns></returns>
        public string RenderPageFromAssembly(Assembly[] assemblies) {
            ...
            foreach (var type in types) {
                if (IsController(type)) {
                    string controllerHtml = RenderController(type);
        ...
        /// <param name="type">The type.</param>
        /// <returns></returns>
        public string RenderController(Type controller) {
            ...
            if (null != methods && methods.Length > 0) {
                foreach (var method in methods) {
                    ...
                    if (null != mType) {
                        html.AppendFormat("<div>{0}</div>", RenderMethod(method));
                    }
                }
                if (html.Length > 0) {
        ...
        /// <returns></returns>
        public string RenderMethod(MethodInfo method) {
            StringBuilder html = new StringBuilder();
            ...
            //parameters
            html.Append(RenderMethodParameters(method.GetParameters(), formId, httpMethod, formAction));            
        ...
        /// <returns></returns>
        public string RenderMethodParameters(ParameterInfo[] parameters, string formId, string method, string action) {
